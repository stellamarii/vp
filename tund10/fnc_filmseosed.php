<?php
	...
	function readfromdb($sortby, $sortorder){
		...
		if($sortby == 0 and $sortorder == 0){
			$stmt = $conn->prepare("SELECT first_name, last_name, role, title, quote FROM person JOIN person_in_movie ON person.person_id = person_in_movie.person_id JOIN movie ON movie.movie_id = person_in_movie.movie_id JOIN quote ON quote.person_in_movie_id = person_in_movie.person_in_movie_id");
		}
		if($sortby == 4){
			if($sortorder == 1){
				$stmt = $conn->prepare("SELECT first_name, last_name, role, title, quote FROM person JOIN person_in_movie ON person.person_id = person_in_movie.person_id JOIN movie ON movie.movie_id = person_in_movie.movie_id JOIN quote ON quote.person_in_movie_id = person_in_movie.person_in_movie_id ORDER BY title");
			} else {
				$stmt = $conn->prepare("SELECT first_name, last_name, role, title, quote FROM person JOIN person_in_movie ON person.person_id = person_in_movie.person_id JOIN movie ON movie.movie_id = person_in_movie.movie_id JOIN quote ON quote.person_in_movie_id = person_in_movie.person_in_movie_id ORDER BY title DESC");
			}
		}

		echo $conn->error;
		$stmt->bind_result($fnamefromdb, $lnamefromdb, $rolefromdb, $titlefromdb, $quotefromdb);
		$stmt->execute();
		$lines = "";
		while($stmt->fetch()){
			$lines .= "\t <tr> \n";
